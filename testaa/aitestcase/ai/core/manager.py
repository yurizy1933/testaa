...
class AIManager:
    ...
    def __init__(self, provider_name: Optional[str] = None, config: Optional[AIConfig] = None):
        ...
        self.cache = AICache(enabled=self.config.enable_cache, ttl=self.config.cache_ttl)
        self.monitor = AIMonitor(enabled=self.config.enable_monitoring, log_level=self.config.log_level)
    ...
    def _get_provider(self, provider_name: str) -> AIProvider:
        ...
        # 传入配置中的额外参数（如果有）
        extra_kwargs = getattr(self.config, 'extra_kwargs', {})
    ...
    def call_chat_completion(
        ...
        **kwargs
        ...
        # 记录调用开始
        call_id = self.monitor.log_call_start(self.provider_name, model, messages)
        ...
        try:
            ...
            # 记录调用失败
            self.monitor.log_call_error(call_id, str(e))
            raise
    ...
    def _generate_cache_key(
        ...
        # 生成唯一键
        key_data = f"{self.provider_name}:{model}:{temperature}:{max_tokens}:{json_mode}:{messages_str}"
